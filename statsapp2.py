...
def extract_fraction(text):
    """
    Returns the FIRST fraction in a string, e.g. '50.0% 3/7' -> '3/7'
    Returns None if no fraction is found.
    """
    match = re.search(r"(\d+\/\d+)", text)
    return match.group(1) if match else None

def detect_court_region(img):
    """
    Automatically detects the vertical region containing the FG% court diagram.
    This works by scanning for RED or BLUE zones in the court.

    Returns a cropped PIL Image of JUST the court area.
    """
    import numpy as np
    ...
    # red-ish areas indicate court
    red_mask = (arr[:, :, 0] > 180) & (arr[:, :, 1] < 120) & (arr[:, :, 2] < 120)

    # blue-ish areas indicate paint/rim
    blue_mask = (arr[:, :, 2] > 150) & (arr[:, :, 0] < 120)

    mask = red_mask | blue_mask

    rows_with_color = mask.any(axis=1)

    ys = np.where(rows_with_color)[0]
    if len(ys) < 10:
        raise ValueError("Could not detect court region in PDF page.")

    top = max(0, ys[0] - 20)
    bottom = min(h, ys[-1] + 20)

    return img.crop((0, top, w, bottom))

def crop_relative(img, left, top, right, bottom):
    """
    Crops using relative percentages inside the given court image.
    left, top, right, bottom are given as 0–1.
    """
    w, h = img.size
    return img.crop((
        int(left * w),
...
FG_ZONES = {
    "At Rim":         (0.33, 0.00, 0.66, 0.22),
    "In Paint":       (0.33, 0.23, 0.66, 0.45),
    "Midrange 2s":    (0.25, 0.46, 0.75, 0.75),
    "Above Break 3s": (0.25, 0.76, 0.75, 0.90),
    "Corner Left":    (0.00, 0.46, 0.25, 0.75),
    "Corner Right":   (0.75, 0.46, 1.00, 0.75),
}

def extract_2s_3s_from_text(full_page_text):
    line2s = None
    line3s = None

    for ln in full_page_text.splitlines():
        ln = ln.strip()
        if ln.startswith("2s:"):
            line2s = extract_fraction(ln)
        elif ln.startswith("3s:"):
            line3s = extract_fraction(ln)

    return line2s, line3s

def parse_fg_region_pdf(uploaded_pdf):
    """
    Given a full TEAM PDF from CBB Analytics, extracts FG% fractions
    for At Rim, In Paint, Midrange, Above Break 3s, Corner 3s (combined),
    plus All 2s and All 3s.
    ...
    for page in pages:
        # FULL PAGE OCR (for 2s/3s)
        full_text = pytesseract.image_to_string(page)

        # Player name + jersey extraction
        header_match = re.search(r"#(\d+)\s+([A-Za-z\.\-\' ]+)", full_text)
        if not header_match:
            continue

        jersey = header_match.group(1).strip()
        name = header_match.group(2).strip()

        # Extract 2s and 3s totals
        all2s, all3s = extract_2s_3s_from_text(full_text)

        # Detect court block
        court = detect_court_region(page)

        zone_results = {}

        for zname, (L, T, R, B) in FG_ZONES.items():
            crop = crop_relative(court, L, T, R, B)
            text = pytesseract.image_to_string(crop)
            frac = extract_fraction(text)
            zone_results[zname] = frac

        # Combine corner 3s
        cL = zone_results.get("Corner Left")
        cR = zone_results.get("Corner Right")
        ...
        mL, aL = frac_to_nums(cL)
        mR, aR = frac_to_nums(cR)

        if (aL + aR) > 0:
            combined_corner = f"{mL + mR}/{aL + aR}"
        else:
            combined_corner = None

        row = {
            "Jersey": jersey,
            "Player": name,
            "At Rim": zone_results.get("At Rim"),
            "In Paint": zone_results.get("In Paint"),
            "Midrange 2s": zone_results.get("Midrange 2s"),
            "Above Break 3s": zone_results.get("Above Break 3s"),
            "Corner 3s": combined_corner,
            "All 2s": all2s,
            "All 3s": all3s,
        }
        rows.append(row)
...
st.header("FG% by Region (CBB Analytics)")

team_pdf = st.file_uploader(
    "Upload CBB Analytics Team PDF (Player Profiles)",
    type=["pdf"],
    key="fg_pdf"
)
...
if team_pdf is not None:
    if not title_text:
        st.error("❗ Please enter a team name before generating the FG% DOCX.")
    elif logo_bytes is None:
        st.error("❗ Please upload a team logo before generating the FG% DOCX.")
    else:
        try:
            df_fg = parse_fg_region_pdf(team_pdf)
        except Exception as e:
...
def build_fg_docx(df_fg, team_name, logo_bytes, header_color):
    doc = Document()
    ...
    run = title.add_run(f"{team_name.upper()} FG% BY REGION")
    run.bold = True
    run.font.size = Pt(14)
    if header_color:
        run.font.color.rgb = header_color
    ...
    categories = [
        ("At Rim", "At Rim"),
        ("In Paint", "In Paint"),
        ("Midrange 2s", "Midrange 2s"),
        ("All 2s", "All 2s"),
        ("Above Break 3s", "Above Break 3s"),
        ("Corner 3s", "Corner 3s"),
        ("All 3s", "All 3s"),
    ]
    ...
    for left_cat, right_cat in pairs:
        ...
        L = table.rows[0].cells[0]
        R = table.rows[0].cells[1]

        for cell, cat in [(L, left_cat), (R, right_cat)]:
            if cat is None:
                continue
            col, title = cat
